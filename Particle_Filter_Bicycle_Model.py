...
max_steering_angle = pi / 4.0 # We do not need to use this value, but keep in mind the limitations of a real car.
bearing_noise = 0.1 
steering_noise = 0.1
distance_noise = 5.0 
...
class robot:
    ...
    def move(self, motion): 
        ...
        #make a new robot copy (after the motion)
        res = robot()
        res.length = self.length
        res.bearing_noise = self.bearing_noise
        res.steering_noise = self.steering_noise
        res.distance_noise = self.bearing_noise
        ...
        if abs(turn) < tolerance_orientation:
            
            #approximately straight line motion
    ...
    def sense(self, add_noise = 1):
        #Z is the measurement with four bearings
        Z = []
...
def particle_filter(motions, measurements, N=1000): # N is tempting
    # --------
    #
    # Make particles
    # 
...
## vector near Ground truth, that is, the
