...
class particles:
    ...
    def get_position(self):
        ...
        for i in range(self.N):
            x += self.data[i].x
            y += self.data[i].y
            # orientation is tricky because it is cyclic. 
...
def run(grid, goal, spath, params, printflag = 0, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        drx = estimate[0] - spath[index][0]     # estimate is obtained by the particle filter
        dry = estimate[1] - spath[index][1]
        
        # u is the robot estimate projectes onto the path segment
        u = (drx * dx + dry * dy)/(dx* dx + dy * dy)
        # CTE is the robot estimate projectes onto the normal of the path segment
