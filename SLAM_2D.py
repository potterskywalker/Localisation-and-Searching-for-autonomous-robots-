# ------------
# 2D-SLAM in a rectolinear world (we avoid non-linearities)
...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    dim = 2 * (N + num_landmarks) # length of path + number of landmarks
    # times two because we are modelling x and y in the same structures
    
    
    # make the constraint information matrix and vector
    Omega = matrix()
    Omega.zero(dim, dim)
    Omega.value[0][0] = 1.0 # initial position
    Omega.value[1][1] = 1.0
    
    Omega = matrix()
    Omega.zero(dim, 1)
    Omega.value[0][0] = world_size/2.0 # initial position
    Omega.value[1][0] = world_size/2.0
    ...
    for k in range(len(data)):
        ...
        # n is the index of the robot pose in the matrix
        n = k * 2
        ...
        for i in range(len(measurement)):
            ...
            # m is the index of the landmark coordinate in the matrix
        ...
        for b in range(2):
                Omega.value[n+b  ][n+b+2] += -1.0/measurement_noise
                Omega.value[n+b+2][n+b  ] += -1.0/measurement_noise
